#!/usr/bin/env bash
# publish.sh — submit com.pens.CurrencyConverter to the AGL PENS Store
...
"$PUSH_SCRIPT" \
  --token    "$TOKEN" \
  --app-id   "com.pens.CurrencyConverter" \
  --name     "Currency Converter" \
  --summary  "Currency Converter application for AGL Store" \
  --category "Education" \
  --repo     ./flatpak_repo
