...
FIELDS = [("Bill amount", "85.50"), ("Tip %", "15"), ("Split between", "2")]

def compute(vals):
    bill = float(vals[0]); pct = float(vals[1]); split = max(1, int(vals[2]))
    tip = bill * pct / 100.0
    total = bill + tip
    return "\n".join([
        "Bill        : {:.2f}".format(bill),
        "Tip ({:.0f}%)   : {:.2f}".format(pct, tip),
        "Total       : {:.2f}".format(total),
        "Split        : {} people".format(split),
        "Per person  : {:.2f}".format(total / split),
        "  (tip each : {:.2f})".format(tip / split),
    ])
