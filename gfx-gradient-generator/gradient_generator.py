...
APP_ID = "com.pens.GradientGenerator"
FIELDS = [("Start hex", "#FF0000"), ("End hex", "#0000FF"), ("Steps", "8")]
...
def compute(vals):
    a = _hex(vals[0]); b = _hex(vals[1]); steps = max(2, min(64, int(vals[2])))
    out = []
    for i in range(steps):
        t = i / (steps - 1)
        rgb = tuple(round(a[j] + (b[j] - a[j]) * t) for j in range(3))
        out.append("#{:02X}{:02X}{:02X}   rgb{}".format(rgb[0], rgb[1], rgb[2], rgb))
    return "\n".join(out)
