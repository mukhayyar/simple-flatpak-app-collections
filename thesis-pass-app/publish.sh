#!/usr/bin/env bash
# publish.sh — submit com.thesis.PassApp to the AGL PENS Store
set -euo pipefail
TOKEN=""
while [[ $# -gt 0 ]]; do
...
cd "$SCRIPT_DIR"
"$PUSH_SCRIPT" --token "$TOKEN" --app-id "com.thesis.PassApp" --name "PassApp" \
  --summary "Minimal PASS-class benchmark bundle" --category "Utility" \
  --repo ./flatpak_repo
