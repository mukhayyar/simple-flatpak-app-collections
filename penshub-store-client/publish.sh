#!/usr/bin/env bash
# publish.sh — submit com.pens.PensHubClient to the AGL PENS Store
set -euo pipefail
TOKEN=""
while [[ $# -gt 0 ]]; do
...
cd "$SCRIPT_DIR"
"$PUSH_SCRIPT" --token "$TOKEN" --app-id "com.pens.PensHubClient" --name "PensHub Store Client" \
  --summary "Browse and install apps from the AGL PENS App Store" \
