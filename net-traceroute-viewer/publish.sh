#!/usr/bin/env bash
# publish.sh — submit com.pens.TracerouteViewer to the AGL PENS Store
...
"$PUSH_SCRIPT" \
  --token    "$TOKEN" \
  --app-id   "com.pens.TracerouteViewer" \
  --name     "Traceroute Viewer" \
  --summary  "Traceroute Viewer application for AGL Store" \
  --category "Network" \
  --repo     ./flatpak_repo
