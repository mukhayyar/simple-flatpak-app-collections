...
APP_ID = "com.pens.AgeCalculator"
FIELDS = [("Birth date (YYYY-MM-DD)", "2000-01-01"), ("As of (blank = today)", "")]

def compute(vals):
    b = datetime.date.fromisoformat(vals[0].strip())
    asof = datetime.date.fromisoformat(vals[1].strip()) if vals[1].strip() else datetime.date.today()
    if asof < b:
        return "The 'as of' date is before the birth date."
    years = asof.year - b.year - ((asof.month, asof.day) < (b.month, b.day))
    days_total = (asof - b).days
    try:
        nb = b.replace(year=asof.year)
    except ValueError:
        nb = b.replace(year=asof.year, day=28)
    if nb < asof:
        try:
            nb = b.replace(year=asof.year + 1)
        except ValueError:
            nb = b.replace(year=asof.year + 1, day=28)
    return "\n".join([
        "Born     : " + b.isoformat(),
        "As of    : " + asof.isoformat(),
        "Age      : {} years".format(years),
        "Total    : {} days  ({} weeks)".format(days_total, days_total // 7),
        "Next b'day in {} days".format((nb - asof).days),
    ])
