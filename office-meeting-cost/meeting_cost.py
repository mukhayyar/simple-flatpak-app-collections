...
APP_ID = "com.pens.MeetingCost"
FIELDS = [("Attendees", "6"), ("Avg hourly rate", "50"), ("Duration (minutes)", "45")]

def compute(vals):
    n = max(1, int(vals[0])); rate = float(vals[1]); mins = float(vals[2])
    hours = mins / 60.0
    per_person = rate * hours
    total = per_person * n
    return "\n".join([
        "Attendees    : {}".format(n),
        "Rate / hour  : {:.2f}".format(rate),
        "Duration     : {:.0f} min ({:.2f} h)".format(mins, hours),
        "Cost / person: {:.2f}".format(per_person),
        "TOTAL COST   : {:.2f}".format(total),
        "Per minute   : {:.2f}".format(total / mins if mins else 0),
    ])
